...
while True:
    ...
    if now.hour in [3, 9, 15, 21]:
        for run in ["00", "06", "12", "18"]:
            if not os.path.exists(f"/mnt/storage_b/data/ocf/solar_pv_nowcasting/nowcasting_dataset_pipeline/NWP/DWD/ICON_Global/{run}/"):
                os.mkdir(f"/mnt/storage_b/data/ocf/solar_pv_nowcasting/nowcasting_dataset_pipeline/NWP/DWD/ICON_Global/{run}/")
            def get_run():
                now = datetime.now()
            ...
            def download_extract_url(url, folder=f"/mnt/storage_b/data/ocf/solar_pv_nowcasting/nowcasting_dataset_pipeline/NWP/DWD/ICON_Global/{run}/"):
        ...
        for run in ["00", "06", "12", "18"]:
            lon_ds = xr.open_mfdataset(f"/mnt/storage_b/data/ocf/solar_pv_nowcasting/nowcasting_dataset_pipeline/NWP/DWD/ICON_Global/{run}/icon_global_icosahedral_time-invariant_*_CLON.grib2", engine="cfgrib")
            lat_ds = xr.open_mfdataset(f"/mnt/storage_b/data/ocf/solar_pv_nowcasting/nowcasting_dataset_pipeline/NWP/DWD/ICON_Global/{run}/icon_global_icosahedral_time-invariant_*_CLAT.grib2", engine="cfgrib")
            lons = lon_ds.tlon.values
            lats = lat_ds.tlat.values
            datasets = []
            for var_3d in var_3d_list:
                paths = [list(glob(f"/mnt/storage_b/data/ocf/solar_pv_nowcasting/nowcasting_dataset_pipeline/NWP/DWD/ICON_Global/{run}/icon_global_icosahedral_pressure-level_*_{str(s).zfill(3)}_*_{var_3d.upper()}.grib2")) for s in range(73)]
                try:
                    ds = xr.concat([xr.open_mfdataset(p, engine="cfgrib", backend_kwargs={'errors': 'ignore'}, combine="nested", concat_dim="isobaricInhPa").sortby("isobaricInhPa") for p in paths], dim="step").sortby("step")
                except Exception as e:
                    print(e)
                    continue
            ...
            for var_2d in var_2d_list:
                ...
                print(var_2d)
                try:
                    ds = xr.open_mfdataset(f"/mnt/storage_b/data/ocf/solar_pv_nowcasting/nowcasting_dataset_pipeline/NWP/DWD/ICON_Global/{run}/icon_global_icosahedral_single-level_*_*_{var_2d.upper()}.grib2", engine="cfgrib", backend_kwargs={'errors': 'ignore'}, combine="nested", concat_dim="step").sortby("step").drop_vars("valid_time")
                except Exception as e:
                    print(e)
                    continue
            ...
            with zarr.ZipStore(
                ...
                ds.chunk({"step": 37, "values": 122500, "isobaricInhPa": -1,}).to_zarr(store, encoding=encoding, compute=True)
            done = False
            while not done:
                try:
                    api.upload_file(
